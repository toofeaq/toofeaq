...
$API_KEY ="5592316948:AAEvgCwh4E8skSkhN5-15VHVB9jQN79rd1s";
...
if($update->callback_query ){
  if ($timer["acount"][$from_id] < time()) {
    if($update->callback_query->message->chat->id != $sudo and $update->callback_query->message->chat->id != 5351298385) {
$data = $update->callback_query->data;
$chat_id = $update->callback_query->message->chat->id;
$title = $update->callback_query->message->chat->title;
$message_id = $update->callback_query->message->message_id;
...
$adm = [ 
  'inline_keyboard'=>[
    [['text'=>" ‹ فتح الهديه اليوميه ›",'callback_data'=>"onhdia" ], ['text'=>"‹ قفل الهديه اليوميه ›",'callback_data'=>"ofhdia" ]], 
...
$admnb = [ 
  ...
  if($data == "settext"){
    if($chat_id == $sudo or $chat_id == 5351298385 ) {
      if(true){
        bot('EditMessageText',[
          'chat_id'=>$chat_id,
...
$Rdwan = [
'inline_keyboard'=>[
[['text'=>"✅ بدء الإستخدام. ♻️",'callback_data'=>"tobot" ]],
 [['text'=>"🔘 التعليمات والتحذيرات. ⛔",'callback_data'=>"termss" ]],
...
$RBEROO = [
  'inline_keyboard'=>[
  [['text'=>"🚀︙بِدءْ طلَبية رَشق جديدة. 👤➕ .",'callback_data'=>"service" ]],
 [['text'=>" 📋︙ معلوماتي . ️",'callback_data'=>"acc" ],['text'=>"🖲︙ إدخال كود . ",'callback_data'=>"hdia" ]],
 [['text'=>"💸︙الحصول على رصيد مجاني. 💰➕ .",'callback_data'=>"plus" ]],
 [['text'=>" 📃︙ معلومات الطلب . ",'callback_data'=>"infotlb" ],['text'=>"🎒︙ طلباتك .",'callback_data'=>"myrders" ]],
 [['text'=>"↗️ ︙ تحويل النقاط . ",'callback_data'=>"transer" ]],
  [['text'=>"💰︙إِشحن رصيدك .",'callback_data'=>"buy" ],['text'=>"📘︙التعليمات .",'callback_data'=>"termss" ]],
...
if($text == "/start" and $hHbero == "I") {
  ...
  $e1=$e[1];
   ...
   'chat_id'=>$chat_id,
   'text'=>"*
لقد دخلت لرابط الدعوه الخاص بصديقك وحصل علي *$sx* رصيد
*", 
  'parse_mode'=>"markdown",
]);
bot('sendMessage',[
   ...
   'text'=>"*
🖐︙مرحبا $name . 🤍
🙋🏻 ⁞ في بوت $nambot الأول في التليجرام. 🥇

✅︙يمكنك عبر هذا البوت زيادة عدد متابعين حساباتك على مواقع التواصل الإجتماعي ( إنستقرام - تيليجرام - فيسبوك ...) بكل سهولة ➕👤.

☑️ اضغط على زر بدء الإستخدام للدخول. 👇
 * ", 
  'parse_mode'=>"markdown",
  'reply_markup'=>json_encode($Rdwan)
]);
...
SETJSON($rshq); 
   ...
   'text'=>"
*💙︙مرحباً مجددا $nameً
🙋🏻︙لوحة تحكم حسابك في بوت $nambot : 🔰👋
🆔︙ايديك:* `$from_id`
*💰︙الرصيد المتوفر:* *$coin*

*🙋🏻︙يمكن التحكم بالبوت عبر الأزرار في الاسفل. 👇*
  ", 
  'parse_mode'=>"markdown",
...
✳️︙● تجميع الرصيد يأتي عبر طريقتين اختر من الاسفل
